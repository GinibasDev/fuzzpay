...
export default function AdminPayinReportPage() {
  ...
  const fetchReport = async () => {
    setIsLoading(true)
    try {
      const params = new URLSearchParams({ type: 'payin' })
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `admin_payin_report_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">Payin Report</h1>
          <p className="text-muted-foreground">Generate and export payin reports</p>
        </div>

...
export default function AdminPayoutReportPage() {
  ...
  const fetchReport = async () => {
    setIsLoading(true)
    try {
      const params = new URLSearchParams({ type: 'payout' })
  ...
  const handleExport = () => {
    ...
    const headers = ["Date", "Transactions", "Total Amount", "Fees", "Net Amount"]
    ...
    link.setAttribute("download", `admin_payout_report_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">Payout Report</h1>
          <p className="text-muted-foreground">Generate and export payout reports</p>
        </div>
